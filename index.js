...
const TWITTER_HANDLE = '' //Your twitter handle
...
// async function test() {
//   const data = await twitterClient.accountsAndUsers.followersList({ screen_name: 'arshtweetbot', count: 3 });
...
axios({
  ...
  async function drawImage(back, img1, img2, img3){
    ...
    Promise.all(jimps).then(data => {
      ...
      data[0].composite(data[3],1250,50); //experiment with it or DM me on Twitter if you want any help
